...
void fetchAndPublish(Publisher* _pub){
    ...
    while((ret = read(_pub->db_fd, header_buf, sizeof(MessageHeader))) > 0){
        ...
        char* data_buf = (char*)malloc((_pub->msg_header.len + sizeof(MessageHeader)) * sizeof(char));
        if(data_buf == NULL){
            perror("Error allocating memory");
            exit(EXIT_FAILURE);
        }
        memset(data_buf, '\0', (_pub->msg_header.len + sizeof(MessageHeader)));
        read(_pub->db_fd, data_buf + sizeof(MessageHeader), _pub->msg_header.len);
        memcpy(data_buf, header_buf, sizeof(MessageHeader));
        // debug //
        printf("%s\n---------------------------\n", data_buf+sizeof(MessageHeader));
        
        if(send(_pub->socket_fd, data_buf, _pub->msg_header.len + sizeof(MessageHeader), 0) < 0){
            printf("Unable to send message to server\n");
        }
        free(data_buf);
        data_buf = NULL;
