...
void heapifyUp(Heap* _heap, int _index) {
    ...
    while (_index > 0 && _heap->heap[_index].header.has_prioriry >
                             _heap->heap[parent].header.has_prioriry) {
...
void heapifyDown(Heap* _heap, int _index) {
    ...
    if (left < _heap->size && _heap->heap[left].header.has_prioriry >
                                  _heap->heap[largest].header.has_prioriry) {
        largest = left;
    }
    if (right < _heap->size && _heap->heap[right].header.has_prioriry >
                                   _heap->heap[largest].header.has_prioriry) {
        largest = right;
    }
    if (largest != _index) {
...
void resizeHeap(Heap* _heap) {
    ...
    Message* new_heap =
        (Message*)realloc(_heap->heap, new_capacity * sizeof(Message));
    if (new_heap == NULL) {
